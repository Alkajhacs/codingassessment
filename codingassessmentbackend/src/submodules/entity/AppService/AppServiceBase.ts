...
import { ServiceOperationResultType } from "../common/ServiceOperationResultType";
import { Brackets, createConnection, EntitySchema, getRepository, Like, ObjectType, Repository, SelectQueryBuilder } from "typeorm";
...
import { RequestModelQuery } from "../common/RequestModelQuery";
import { ConditionalOperation } from "../common/conditionOperation";
...
export default class AppService<TEntity extends EntityBase, TDto extends DtoBase>{
  ...
  async getone(b: any) {
    ...
    if(parseInt(b)) {
      result= await this.genericRepository.createQueryBuilder('problem_entity')
      .select()
      .where("id= :id",{id: `${b}`})
      .getMany()
    }
      else
      {
        result= await this.genericRepository.createQueryBuilder('problem_entity')
        .select()
        .where("problem_entity.one_Name LIKE ':n'",{n: b})
        .getMany();
      }
